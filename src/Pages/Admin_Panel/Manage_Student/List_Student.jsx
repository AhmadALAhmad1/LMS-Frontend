...
export default function List_Student(props) {
  ...
  const fetchdata = async () => {
    await API.post(`stu-sec-class`).then((res) => {
      ...
      <div className="flexRowStu">
        {state.student &&
          state.student.map((student) => (
            <Link
              onClick={() => props.history.push(`/student/info/${student.id}`)}
            >
              <div
                key={student.id}
                class="container mt-5 d-flex justify-content-center widthAndDiv"
              >
                <div class="card p-3">
                  <div class="d-flex align-items-center">
                    <div class="image">
                      <img
                        src={`http://127.0.0.1:8000/api/image/${student.image}`}
                        class="rounded widthheightimg"
                      />
                    </div>
                    <div class="ml-3 w-100">
                      <h4 class="mb-0 mt-0">{student.fname}</h4>{" "}
                      <h5>{student.lname}</h5>
